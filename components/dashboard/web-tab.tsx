 ...
 * Description: Tab de Web Analytics - Tooltips arreglados + KPIs útiles
 */
...
export function WebTab({ data }: WebTabProps) {
  ...
  // Filtrar campaigns por medium
  const filteredCampaigns = useMemo(() => {
    let filtered = utmCampaigns
    if (mediumFilter !== 'All') {
      filtered = filtered.filter(c => (c.medium || '(not set)') === mediumFilter)
    }
    return filtered.sort((a, b) => sortDir === 'desc' ? b.sessions - a.sessions : a.sessions - b.sessions)
  }, [utmCampaigns, mediumFilter, sortDir])
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left py-3 px-4 font-semibold text-neutral-400 group relative">
                    Source
                    <span className="ml-1 text-neutral-500 cursor-help">ⓘ</span>
                    <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block bg-card border border-border rounded-lg p-2 text-xs text-foreground whitespace-nowrap z-50 shadow-lg">
                      Origin of the traffic (e.g., google, linkedin, x)
                    </div>
                  </th>
                  <th className="text-left py-3 px-4 font-semibold text-neutral-400 group relative">
                    Medium
                    <span className="ml-1 text-neutral-500 cursor-help">ⓘ</span>
                    <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block bg-card border border-border rounded-lg p-2 text-xs text-foreground whitespace-nowrap z-50 shadow-lg">
                      Type of traffic (organic, post, referral, email)
                    </div>
                  </th>
                  <th className="text-left py-3 px-4 font-semibold text-neutral-400 group relative">
                    Campaign
                  ...
                  <th className="text-left py-3 px-4 font-semibold text-neutral-400 group relative">
                    ...
                    <span className="ml-1 text-neutral-500 cursor-help">ⓘ</span>
                    <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block bg-card border border-border rounded-lg p-2 text-xs text-foreground whitespace-nowrap z-50 shadow-lg">
                      Unique identifier from utm_id parameter (e.g., 20260305)
                    </div>
                  </th>
                  <th 
                  ...
                  <th className="text-right py-3 px-4 font-semibold text-neutral-400 group relative">
                    % Total
                    <span className="ml-1 text-neutral-500 cursor-help">ⓘ</span>
